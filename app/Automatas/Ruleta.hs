...
foreign import javascript unsafe "let isWin = $1; let voiceUrl = $2; let music = document.getElementById('sound-music'); if (music && !music.paused) { music.volume = 0.05; }; let sfx = document.getElementById(isWin ? 'sound-win' : 'sound-lose'); if (sfx) { sfx.volume = 1.0; sfx.currentTime = 0; sfx.play().catch(e => {}); }; setTimeout(() => { let v = new Audio(voiceUrl); if (v) { if (window.currentVoice) { window.currentVoice.pause(); }; window.currentVoice = v; v.volume = 0.7; let ctx = window.audioCtx; let gainNode = window.gainNode; try { if (!ctx) { ctx = new (window.AudioContext || window.webkitAudioContext)(); gainNode = ctx.createGain(); gainNode.gain.value = 1.8; gainNode.connect(ctx.destination); window.audioCtx = ctx; window.gainNode = gainNode; }; if (ctx.state === 'suspended') { ctx.resume(); }; let src = ctx.createMediaElementSource(v); src.connect(gainNode); } catch (e) {}; v.play().catch(e => {}); v.onended = () => { if (music) music.volume = 0.30; }; }; }, 300); setTimeout(() => { if (music) music.volume = 0.30; }, 5000);"
  js_playResultAudio :: Bool -> JSVal -> IO ()
#else
type JSVal = String
  ...
  HandleResult num -> do
    ...
    newModel <# do
      if payout > 0
        then do
          js_triggerConfetti
          js_playResultAudio True (unsafeCoerce (ms (speechPath num) :: MisoString))
          threadDelay 5000000
          return CloseWinPopup
        else do
          js_playResultAudio False (unsafeCoerce (ms (speechPath num) :: MisoString))
          return NoOp
  ...
  div_ [ class_ "container" ]
    ...
    , div_ [ class_ "game-area glass-panel" ]
        ...
        , div_ [ class_ "board-container" ]
            ...
            , div_ [ class_ "ruleta-actions-row" ]
                [ button_
                    ([ class_ "btn-clear"
                     , onClick ClearBets
                     ] ++ if spinning m || null (activeBets m)
